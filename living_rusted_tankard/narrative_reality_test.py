...
class NarrativeRealityTest:
    ...
    def run_narrative_test(self) -> Dict[str, Any]:
        ...
        narrative_commands = [
            ...
            ("I approach the bar and greet the bartender", "social", "First NPC contact"),
            ("I ask about this establishment", "information", "World building"),
            
            # Exploration and discovery
            ("I'm curious about the woman in the corner", "observation", "NPC interest"),
            ("I walk over and ask if I can sit with her", "social", "Relationship building"),
            ("I ask her about what she's working on", "social", "Story thread discovery"),
            
            # Building relationships
            ("I tell her I've traveled many trade routes", "personal", "Sharing expertise"),
            ("I offer to help with her business problems", "helpful", "Active assistance"),
            ("I share my knowledge of northern routes", "expertise", "Providing value"),
            
            # Deepening connections
            ("I return the next day and greet Sarah", "continuity", "Relationship persistence"),
            ("I ask Gene about the tavern's history", "lore", "World depth"),
            ("I check on how Sarah's business is doing", "follow_up", "Consequence tracking"),
            
            # Mystery and complexity
            ("I notice the man in the corner and his strange coins", "mystery", "Observation skills"),
            ("I carefully approach Marcus and ask to join him", "social_risk", "Trust building"),
            ("I ask Marcus about his travels", "personal", "Character development"),
            
            # Difficult choices
            ("I discuss Marcus's offer with Gene privately", "complex", "Moral complexity"),
            ("I try to help Marcus while protecting Gene's neutrality", "values", "Character consistency"),
            ...
            ("I leave something valuable for future travelers", "legacy", "Lasting impact"),
        ]
        ...
        for i, (command, category, intent) in enumerate(narrative_commands, 1):
            ...
            narrative_quality = self._analyze_narrative_quality(command, message, category)
    ...
    def _analyze_narrative_quality(self, command: str, response: str, category: str) -> Dict[str, Any]:
        ...
        immersive_indicators = ['you see', 'you notice', 'you feel', 'the atmosphere', 'warmly', 'carefully']
        quality['immersive'] = any(indicator in response_lower for indicator in immersive_indicators)
        
        # Check for character awareness (NPCs mentioned by name)
        character_indicators = ['gene', 'sarah', 'marcus', 'bartender']
        quality['character_aware'] = any(char in response_lower for char in character_indicators)
        
        # Check for story advancement
        story_indicators = ['tells you', 'mentions', 'explains', 'reveals', 'suggests']
        quality['story_advancing'] = any(indicator in response_lower for indicator in story_indicators)
        
        # Check for emotional resonance
        emotion_indicators = ['grateful', 'pleased', 'concerned', 'interested', 'surprised']
        quality['emotionally_resonant'] = any(emotion in response_lower for emotion in emotion_indicators)
        
        # Check for world building
        world_indicators = ['tavern', 'establishment', 'history', 'tradition', 'community']
        quality['world_building'] = any(indicator in response_lower for indicator in world_indicators)
        ...
        for npc in npcs:
            if npc in message:
                if npc not in self.relationship_tracking:
                    self.relationship_tracking[npc] = {'interactions': 0, 'depth': 'shallow'}
    ...
    def _analyze_overall_narrative_performance(self) -> Dict[str, Any]:
        ...
        immersive_responses = sum(1 for moment in self.narrative_log 
                                if moment.get('narrative_quality', {}).get('immersive', False))
        
        character_aware_responses = sum(1 for moment in self.narrative_log 
                                      if moment.get('narrative_quality', {}).get('character_aware', False))
        
        story_advancing_responses = sum(1 for moment in self.narrative_log 
                                      if moment.get('narrative_quality', {}).get('story_advancing', False))
        
        success_rate = sum(1 for moment in self.narrative_log if moment.get('success', False)) / total_moments * 100
        
        return {
            'success_metrics': {
                'basic_success_rate': success_rate,
                'immersion_rate': (immersive_responses / total_moments * 100) if total_moments > 0 else 0,
                'character_awareness_rate': (character_aware_responses / total_moments * 100) if total_moments > 0 else 0,
                'story_advancement_rate': (story_advancing_responses / total_moments * 100) if total_moments > 0 else 0,
            },
    ...
    def print_narrative_analysis(self, results: Dict[str, Any]):
        ...
        print(f"📊 NARRATIVE PERFORMANCE METRICS:")
        ...
        if relationships:
            for npc, data in relationships.items():
                print(f"   {npc.title()}: {data['interactions']} interactions ({data['depth']} relationship)")
        else:
        ...
        if metrics['character_awareness_rate'] < 30:
            print("   • Low character awareness - NPCs not sufficiently present in responses")
        ...
        if not relationships:
            print("   • No relationship tracking - system doesn't remember character interactions")
...
def main():
    ...
    print(f"Target for story-driven gameplay: 75%+")
