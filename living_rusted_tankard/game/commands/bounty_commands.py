...
def view_bounty_command(game_state: 'GameState', args: List[str]) -> Dict[str, Any]:
    ...
    if not args:
        return {"success": False, "message": "Usage: bounty view <bounty_id_or_partial_title>"}
    ...
    for i, obj in enumerate(bounty_obj.objectives):
        ...
        message_lines.append(f"  {status_marker} {obj.description} ({obj.current_progress}/{obj.required_progress})")
    ...
    if bounty_obj.rewards.reputation:
        rep_str = ", ".join([f"{val} with {key}" for key, val in bounty_obj.rewards.reputation.items()])
        message_lines.append(f"  Reputation: {rep_str}")
    if bounty_obj.rewards.xp:
        message_lines.append(f"  XP: {bounty_obj.rewards.xp}")
    if not any([bounty_obj.rewards.gold, bounty_obj.rewards.items, bounty_obj.rewards.reputation, bounty_obj.rewards.xp]):
...
def accept_bounty_command(game_state: 'GameState', args: List[str]) -> Dict[str, Any]:
    ...
    if not args:
        return {"success": False, "message": "Usage: bounty accept <bounty_id_or_partial_title>"}
    ...
    if not found_bounty_dict:
        return {"success": False, "message": f"Available bounty matching '{term}' not found or prerequisites not met."}
    ...
    success, message = bounty_manager.accept_bounty(player.id, bounty_id_to_accept, game_state.clock.current_time_hours)
...
def complete_bounty_command(game_state: 'GameState', args: List[str]) -> Dict[str, Any]:
    ...
    if not args:
        return {"success": False, "message": "Usage: bounty complete <bounty_id_or_partial_title>"}
    ...
    if not found_bounty_dict:
        return {"success": False, "message": f"Active bounty matching '{term}' not found."}
    ...
    active_bounty_instance = bounty_manager.managed_bounties_state.get(bounty_id_to_complete)
    if active_bounty_instance:
        all_objectives_complete = True
        for i, obj in enumerate(active_bounty_instance.objectives):
            if not obj.is_completed:
                # Try to auto-complete simple "find" or "deliver to self" objectives for testing
                if "find" in obj.description.lower() or "deliver" in obj.description.lower():
                     # Simulate finding/delivering for the demo
                    bounty_manager.update_objective_progress(player.id, bounty_id_to_complete, i, obj.required_progress)
                else: # For other objectives, assume they need external completion
                    all_objectives_complete = False
        if not active_bounty_instance.are_all_objectives_completed() and all_objectives_complete:
             # Re-check after potential auto-completion
             if not active_bounty_instance.are_all_objectives_completed():
                return {"success": False, "message": f"Objectives for '{active_bounty_instance.title}' are not yet complete."}


    success, message = bounty_manager.complete_bounty(player.id, bounty_id_to_complete, game_state)
...
def list_active_bounties_command(game_state: 'GameState', args: List[str]) -> Dict[str, Any]:
    ...
    for bounty_id in player.active_bounty_ids:
        ...
        if bounty:
            message_lines.append(f"  ID: {bounty.id} - Title: {bounty.title} (Status: {bounty.status.value})")
...
def list_completed_bounties_command(game_state: 'GameState', args: List[str]) -> Dict[str, Any]:
    ...
    for bounty_id in player.completed_bounty_ids:
        bounty = bounty_manager.view_bounty(bounty_id) # Gets the managed state (should be COMPLETED)
        if bounty:
            ...
            if b_def:
                 message_lines.append(f"  ID: {b_def.id} - Title: {b_def.title} (Status: From Definition - Likely Completed)")
            else:
                message_lines.append(f"  ID: {bounty_id} - (Error: Details not found)")
        message_lines.append("-" * 10)
