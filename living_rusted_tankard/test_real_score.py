...
def test_real_phase1_score():
    ...
    active_relationships = sum(1 for r in relationships.values() if r.get('interactions', 0) > 0)
    ...
    print(f"   Success Rate: {successful_actions}/{len(story_actions)} ({successful_actions/len(story_actions)*100:.1f}%)")
