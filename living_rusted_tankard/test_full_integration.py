...
def test_phase4_narrative_engine(game_state: GameState, integration: PhaseIntegration):
    ...
    assert hasattr(game_state, 'narrative_orchestrator'), "Narrative orchestrator not found"
    ...
    for thread in threads:
        print(f"  - {thread.title} ({thread.type.value}) - Tension: {thread.tension_level}")
...
def test_integration_flow(game_state: GameState, integration: PhaseIntegration):
    ...
    game_state.event_bus.emit(Event(
        ...
        {'old_room': old_room, 'new_room': 'tavern_upstairs'}
    ))
    
    # Check atmosphere updated
    current_atmosphere = integration.atmosphere_manager.get_current_atmosphere()
    print(f"   ✓ Atmosphere updated for new room")
    ...
    if 'bartender' in game_state.npcs:
        ...
        print(f"   ✓ NPC interaction processed through all systems")
    ...
    print(f"   ✓ All systems updated with time progression")
    ...
    print(f"   ✓ Command processed with enhancements")
