...
def test_with_forced_npcs():
    ...
    for cmd, desc in test_commands:
        ...
        if result['success']:
            ...
            if any(word in full_msg.lower() for word in 
                   ['rep:', 'memory', 'remember', 'relationship', 'trust', 
                    'mood', 'quest', 'consequence', 'story']):
                narrative_moments.append((cmd, desc))
                print(f"  🎭 Narrative moment!")
        else:
    ...
    if hasattr(game, 'character_memory_manager'):
        ...
        if gene_memory:
            ...
            for i, mem in enumerate(gene_memory.memories[:3]):
                print(f"   Memory {i+1}: {mem.interaction_type} - {mem.player_action[:50]}...")
