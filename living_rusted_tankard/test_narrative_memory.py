...
from core.narrative.character_memory import CharacterMemoryManager, Memory, RelationshipLevel
from core.narrative.character_state import CharacterStateManager, Mood, Concern, ConcernType, Goal
...
def test_character_memory():
    ...
    grim_memory.add_interaction_memory(
        "Player ordered an ale",
        {"item": "ale", "price": 5}
    )
    
    grim_memory.add_interaction_memory(
        "Had a conversation about the weather",
        {"topic": "weather", "mood": "friendly"}
    )
...
def test_character_state():
    ...
    mira_state.add_concern(Concern(
    ...
    print(f"After hearing about theft:")
    print(f"  Mood: {mira_state.mood.value}")
    print(f"  Primary concern: {mira_state.concerns[0].description if mira_state.concerns else 'None'}")
    print(f"  Status: {mira_state.get_status_description()}\n")
    
    # Add a goal
    mira_state.add_goal(Goal(
        description="Hire a guard for the shop",
        priority=0.9,
        progress=0.0,
        required_actions=["find trustworthy guard", "negotiate payment", "set schedule"]
    ))
    ...
    for i in range(3):
        mira_state.tick()
        print(f"  Tick {i+1}: stress={mira_state.stress:.2f}, energy={mira_state.energy:.2f}")
...
def test_integration():
    ...
    interactions = [
        ...
        ("Player shared news about bandits", {"topic": "danger", "location": "forest road"})
    ]
    ...
    for description, details in interactions:
        ...
        if "danger" in details.get("topic", ""):
            ...
            state.mood_modifiers["good_sale"] = (0.3, datetime.now() + timedelta(hours=1))
        
        state.update_mood()
        print(f"  Mood: {state.mood.value}, Relationship: {memory.get_relationship_level()}")
