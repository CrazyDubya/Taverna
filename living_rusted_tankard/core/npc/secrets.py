...
class EnhancedSecret:
    ...
    def add_evidence(self, evidence: Evidence) -> None:
        ...
        self.revelation_progress = min(1.0, 
            self.revelation_progress + evidence.revelation_power * 0.5)
        self._update_state()
    ...
    def investigate(self, investigator_id: str, skill_level: float = 0.5) -> List[Evidence]:
        ...
        for evidence in self.evidence_trail:
            ...
            if random.random() < base_chance:
                ...
                self.revelation_progress = min(1.0,
                    self.revelation_progress + evidence.get_revelation_value())
    ...
    def create_false_evidence(self, description: str, 
                            evidence_type: EvidenceType = EvidenceType.DOCUMENT) -> Evidence:
    ...
    def get_discovery_risk(self) -> float:
        ...
        discovered_evidence = sum(
            1 for ev in self.evidence_trail 
            if len(ev.discovered_by) > 0
        )
...
class SecretGenerator:
    ...
    @staticmethod
    def generate_secret(npc_data: Dict[str, Any], 
                       secret_type: Optional[SecretType] = None) -> EnhancedSecret:
        ...
        # Get random template
        templates = secret_templates.get(secret_type, [
            ("Has a dark secret", 0.5, 0.5)
        ])
        ...
        secret.evidence_trail.extend(
            SecretGenerator._generate_evidence_trail(secret)
        )
        
        # Add consequences
        secret.consequences.extend(
            SecretGenerator._generate_consequences(secret)
        )
    ...
    def _generate_consequences(secret: EnhancedSecret) -> List[SecretConsequence]:
        ...
        # Base consequence
        consequences.append(SecretConsequence(
