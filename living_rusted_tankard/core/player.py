...
class PlayerState(BaseModel):
    ...
    def _update_hunger_thirst(self, game_time: float) -> None: 
        ...
        if hours_since_ate > 6: self.energy = max(0.0, self.energy - 0.05) 
        if hours_since_drank > 4: self.energy = max(0.0, self.energy - 0.1) 
    
    def add_effect(self, effect_id: str, stat_affected: str, modifier: Any, duration_hours: float, current_game_time: float):
        ...
        new_effect = ActiveEffect(effect_id=effect_id, stat_affected=stat_affected, modifier=modifier, expiry_time=expiry_time)
        self.active_effects.append(new_effect)

        if duration_hours <= 0.01: 
            if stat_affected == "heal": setattr(self, "current_stamina", min(self.get_stat("max_stamina"), self.current_stamina + modifier)) 
            self.active_effects = [eff for eff in self.active_effects if eff.effect_id != effect_id]
    ...
    def get_stat_modifier(self, stat_name: str) -> Any:
        total_modifier = 0
        if stat_name in ["resolve", "max_stamina", "current_stamina", "happiness", "luck_modifier", "hunger_reduction", "heal", "courage", "inebriation", "energy"]: 
    ...
    def get_status_effects(self) -> List[str]:
        display_effects = []
        if self.tiredness > 0.8: display_effects.append("Exhausted")
        elif self.tiredness > 0.5: display_effects.append("Tired")
        if self.energy < 0.3: display_effects.append("Drained")
        elif self.energy < 0.6: display_effects.append("Fatigued")
        if self.rest_immune: display_effects.append("Sleepless")

        for effect in self.active_effects:
            effect_name = effect.effect_id.replace("_from_", " (").replace("_", " ").title() + ")" if "_from_" in effect.effect_id else effect.effect_id.replace("_", " ").title()
    ...
    def consume_item_and_apply_effects(self, item: Item, current_game_time: float) -> bool:
        ...
        if item.effects:
            for effect_key, effect_value in item.effects.items(): # type: ignore
                if effect_key == "duration_hours": continue 
                if effect_key == "random_effect" and effect_value is True: 
                    self.add_effect(effect_id=f"{item.id}_random_effect", stat_affected="random", modifier=True, 
                                    duration_hours=item.effects.get("duration_hours", 0.25), current_game_time=current_game_time) # type: ignore
                else: 
