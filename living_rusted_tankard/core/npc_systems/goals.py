...
class GoalGenerator:
    ...
    @staticmethod
    def generate_goals_for_npc(npc_data: Dict[str, Any],
        ...
        for motivation in psychology.motivations[:3]:  # Top 3 motivations
            if motivation.type == MotivationType.WEALTH:
                ...
                goals.append(GoalGenerator._create_revenge_goal(
                    npc_data["id"], 
                    motivation.target
                ))
    ...
    def _create_food_goal(npc_id: str) -> Goal:
        ...
        # Add steps
        goal.add_step(GoalStep(
            description="Go to kitchen or bar",
            action_required=Action("Move to food source", "move", 
                                 {"destination": "kitchen"}),
            time_estimate=0.1
        ))
        ...
        goal.add_step(GoalStep(
            ...
            action_required=Action("Get food", "interact", 
                                 {"target": "food_source"}),
    ...
    def _create_rest_goal(npc_id: str) -> Goal:
        ...
        goal.add_step(GoalStep(
            ...
            action_required=Action("Find rest spot", "move", 
    ...
    def _create_merchant_goals(npc_id: str) -> List[Goal]:
        ...
        profit_goal.add_step(GoalStep(
            ...
            action_required=Action("Advertise", "speak", 
                                 {"dialogue": "Fine goods for sale!"}),
            time_estimate=1.0
        ))
        ...
        supplier_goal.add_step(GoalStep(
    ...
    def _create_guard_goals(npc_id: str) -> List[Goal]:
        ...
        order_goal.add_step(GoalStep(
            ...
            action_required=Action("Investigate", "observe", 
    ...
    def _create_revenge_goal(npc_id: str, target: Optional[str]) -> Goal:
        ...
        goal.add_step(GoalStep(
            ...
            action_required=Action("Investigate", "investigate", 
...
class NPCAgency:
    ...
    def _prioritize_goals(self) -> None:
        ...
        self.goals.sort(
            key=lambda g: g.calculate_priority(context),
            reverse=True
        )
    ...
    def execute_step(self, goal_id: str, step_index: int, 
                    success: bool) -> None:
    ...
    def _find_alternative_approach(self, goal: Goal, 
                                  failed_step: GoalStep) -> None:
        ...
        if resource in self.resources:
            ...
            for goal in self.goals:
                if goal.status == GoalStatus.BLOCKED:
                    ...
                    if all(self.resources.get(r, 0) >= amt 
                          for r, amt in required.items()):
                        goal.status = GoalStatus.ACTIVE
                        goal.blockers = [b for b in goal.blockers 
                                       if "resource" not in b.lower()]
    ...
    def evaluate_goal_progress(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "blocked_goals": len([g for g in self.goals 
    ...
    def generate_new_goals(self, context: Dict[str, Any]) -> List[Goal]:
        ...
        for potential in potential_goals:
            ...
            similar = any(
                g.category == potential.category and 
                g.type == potential.type
                for g in self.goals
            )
