...
class RelationshipWeb:
    ...
    def set_relationship(self, npc1: str, npc2: str, relationship: Relationship,
                        rel_type: RelationshipType = RelationshipType.ACQUAINTANCE) -> None:
        """Set relationship between NPCs."""
        key = self._get_relationship_key(npc1, npc2)
        self.relationships[key] = relationship
        self.relationship_types[key] = rel_type
    
    def create_relationship(self, npc1: str, npc2: str, 
                          rel_type: RelationshipType,
                          trust: float = 0.5,
                          affection: float = 0.5,
                          respect: float = 0.5) -> Relationship:
    ...
    def create_conflict(self, conflict_type: ConflictType,
                       participants: List[str],
                       description: str,
                       root_cause: str,
                       intensity: float = 0.5) -> Conflict:
    ...
    def create_alliance(self, alliance_type: AllianceType,
                       members: List[str],
                       description: str,
                       purpose: str,
                       strength: float = 0.5) -> Alliance:
            ...
            for npc2 in members[i+1:]:
                ...
                if not rel:
                    rel = self.create_relationship(
                        npc1, npc2, RelationshipType.ALLY
                    )
    ...
    def spread_gossip(self, source_npc: str, secret: Secret, 
                     reliability: float = 0.8) -> Set[str]:
    ...
    def record_social_event(self, event_type: str, participants: List[str],
                          location: str, description: str,
                          public: bool = False) -> SocialEvent:
