    ...
    def is_met(self, game_state: Any) -> bool:
        ...
        try:
            if self.condition_type == "reputation":
                ...
                current_room = getattr(game_state.room_manager.current_room, 'id', '') if game_state.room_manager.current_room else ''
                if self.operator == "==":
...
class StoryThread:
    ...
    def __init__(self, thread_id: str, title: str, thread_type: ThreadType, 
                 description: str, priority: ThreadPriority = ThreadPriority.LOW):
                ...
                for npc_id, change in change_data.items():
                    if hasattr(game_state, 'reputation_network'):
                        ...
                        game_state.reputation_network.record_player_action(
                            "story_progression", "stage_completed", [npc_id],
            ...
            if reward_type == "gold":
                ...
                if hasattr(game_state, 'reputation_network'):
                    for npc_id in self.involved_npcs:
                        game_state.reputation_network.record_player_action(
                            "story_heroics", "completed", [npc_id],
    ...
    def get_next_suggested_action(self) -> Optional[str]:
        ...
        unmet_conditions = [
            cond for cond in current_stage.conditions 
...
def create_business_thread(player_gold: int, merchant_npc_id: str) -> StoryThread:
    ...
    stage2 = ThreadStage(
        ...
        world_changes={
            "reputation_change": {merchant_npc_id: 0.5}
        },
