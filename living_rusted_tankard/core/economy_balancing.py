    ...
    def update_economic_events(self, current_time: datetime, player_tier: ProgressionTier) -> Optional[Dict[str, Any]]:
        ...
        if random.random() < 0.05:  # 5% chance per update
            available_events = [
                ...
                if event.min_tier.value <= player_tier.value and 
                event.id not in [ae.id for ae in self.active_events]
            ]
            ...
            "active_events": [
                {
                    ...
                    "time_remaining": (event.active_until - datetime.now()).total_seconds() / 3600
                    if event.active_until else 0
                }
                for event in self.active_events
            ],
            "price_modifiers": {
