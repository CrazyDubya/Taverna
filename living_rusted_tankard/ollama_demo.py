...
class OllamaClient:
    ...
    def generate(self, prompt: str, model: str = "llama3.2:latest", temperature: float = 0.7) -> str:
...
class GameNarrator:
    ...
    def describe_scene(self, location: str, time_of_day: str, npcs: List[str]) -> str:
        ...
        prompt = f"""You are a narrator for a fantasy tavern game. Describe this scene vividly in 2-3 sentences:
Location: {location}
Time: {time_of_day}
...
Keep it atmospheric and immersive. Focus on sights, sounds, and smells."""
        ...
        return response or f"You find yourself in {location}. It's {time_of_day} and you see {', '.join(npcs)}."
    
    def narrate_action(self, action: str, actor: str, target: Optional[str] = None) -> str:
        """Narrate an action dynamically"""
        prompt = f"""You are a game narrator. Describe this action in 1-2 sentences:
Actor: {actor}
Action: {action}
...
class DynamicAIPlayer:
    ...
    def __init__(self, name: str, personality: AIPlayerPersonality, ollama_client: OllamaClient, model: str = "llama3.2:latest"):
        self.name = name
    ...
    def decide_action(self, game_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        context = f"""You are {self.name}, an AI character in a fantasy tavern game.
...
Example: {{"command": "look", "reasoning": "I want to see what mysteries this place holds."}}"""
        ...
        fallback_actions = {
            AIPlayerPersonality.CURIOUS_EXPLORER: ("look", "I need to explore this place!"),
            AIPlayerPersonality.CAUTIOUS_MERCHANT: ("status", "Let me check my resources first."),
            AIPlayerPersonality.SOCIAL_BUTTERFLY: ("look", "Who can I talk to here?"),
            AIPlayerPersonality.MYSTERIOUS_WANDERER: ("wait", "I shall observe in silence...")
        }
    ...
    def speak(self, context: str) -> str:
        ...
        prompt = f"""You are {self.name}, a character with this personality: {personality_desc}
Context: {context}
...
def run_ollama_demo():
    ...
    test_response = ollama.generate("Say 'Hello Tavern' if you can hear me", temperature=0.1)
    ...
    game_state = {
        ...
        "time_of_day": "evening",
        "npcs_present": ["Gene the Bartender", "Old Sailor Jim", "Mysterious Hooded Figure"],
        "player_gold": 100,
        "player_health": 100
    }
    ...
    for ai_player in ai_players:
        ...
        for turn in range(2):
            ...
            print(f"   💭 Reasoning: \"{decision.get('reasoning', 'I act on instinct')}\"")
            ...
            action_narration = narrator.narrate_action(
                ...
                ai_player.name,
                decision.get('command', '').split()[1] if len(decision.get('command', '').split()) > 1 else None
            )
