...
from tests.fixtures.database_fixtures import (
    temp_database, isolated_db_session, database_transaction_manager,
    populated_test_database, database_cleaner
)
...
class TestDatabaseIsolation:
    ...
    def test_database_transaction_manager(self, database_transaction_manager):
        ...
        with database_transaction_manager.transaction() as conn:
            conn.execute("INSERT INTO players (name, gold) VALUES (?, ?)", ("TransactionTest", 200))
    ...
    def test_database_transaction_rollback(self, database_transaction_manager):
        ...
        try:
            with database_transaction_manager.transaction() as conn:
                conn.execute("INSERT INTO players (name, gold) VALUES (?, ?)", ("RollbackTest", 300))
                # Force an error
    ...
    def test_database_savepoints(self, database_transaction_manager):
        ...
        with database_transaction_manager.transaction() as conn:
            conn.execute("INSERT INTO players (name, gold) VALUES (?, ?)", ("SavepointTest1", 100))
            
            try:
                with database_transaction_manager.savepoint("test_savepoint") as conn:
                    conn.execute("INSERT INTO players (name, gold) VALUES (?, ?)", ("SavepointTest2", 200))
    ...
    def test_populated_database_content(self, populated_test_database):
        ...
        # Check sessions
        sessions = populated_test_database.execute("SELECT id, player_name FROM sessions")
        assert len(sessions) >= 2
    
    def test_database_cleaner_functionality(self, isolated_db_session, database_cleaner):
        ...
        # Verify data exists
        players = isolated_db_session.execute("SELECT * FROM players WHERE name = 'CleanerTest'")
        npcs = isolated_db_session.execute("SELECT * FROM npcs WHERE name = 'CleanerNPC'")
...
class TestDatabaseFixtureIntegration:
    ...
    def test_fixture_combination(self, populated_test_database, database_transaction_manager):
